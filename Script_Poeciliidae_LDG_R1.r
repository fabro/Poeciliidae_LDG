...
### Compute maximum branch lenght (MBL) across the multiphylo object ###
# Create an empty matrix to save the MBL values. This matrix must have the same rows as the PAM.
pam_max_age_trees <- matrix(nrow = 6551, ncol = 1000, byrow = FALSE)
# Loop to calculate MBL across the multiphylo object
for (t in 1:length(trees)) {
	tree_phy <- trees[[t]]
	print(t)
# Match PAM species & tree species
	data <- match.phylo.comm(tree, poe_pam)
# Clean community matrix & tree
	pam_phy <- data$comm
	tree_phy <- data$phy
	tree_phy$edge.length <- tree_phy$edge.length*100
# Get edge length values of each tree species
# The next two code lines are from Liam Revell (http://blog.phytool.org/2013/10/finding-edge-lengths-of-all-terminal.html)
	n <- length(tree_phy$tip.label)
	edge_length <- setNames(tree_phy$edge.length[sapply(1:n, function(x,y)which(y==x),y=tree_phy$edge[,2])],tree_phy$tip.label)
# Calculate MBL for all grid cells
	pam_max_age <- NULL
	for (i in 1:nrow(pam_phy)) {
		pixel <- t(subset(pam_phy[i,]))
		spp_pixel <- row.names(subset(pixel, pixel >0))
		spp_pixel_length <- subset(edge_length, names(edge_length) %in% spp_pixel)
		pam_max_age[i] <- (max(spp_pixel_length))
	}
# Save MBL values in the matrix
	pam_max_age_trees[,t] <- pam_max_age 
	write.table(pam_max_age_trees, "MBL_pam.txt",sep = ",")
}
# Estimate the average MBL across the matrix
avg_max_age <- apply(pam_max_age_trees, 1, mean, na.rm = TRUE)

### Prepare MPD results ###
# Rasterize MBL data
avg_max_age_ras <- rasterize(poe_pam[,c(1,2)], PAM.AHULL.MEDG$Richness_Raster, avg_max_age)
# Save MBL raster as tif
writeRaster(avg_max_age_ras, filename = "maximum_age_50km_256spp.tif", format = "GTiff")
# Save MBL data as RData
save(avg_max_age, file = "MBL_256spp.RData")
...
# Note the multiplication by 100 because the phylogeny of Reznick et al. (2017) is in decimal units
		equal_splits <- picante::evol.distinct(tree, 
                                  type = "equal.splits",
                                  scale = FALSE,
...
"SR ~ BasinArea + TempSeas + PrepSeas + NPP + TempAnom + PrepAnom + EvolTime + SpecRate",
"SpecRate ~ BasinArea + TempSeas + PrepSeas + TempAnom + PrepAnom + TH8 + EvolTime",
"EvolTime ~ TempAnom + PrepAnom",
"NPP ~ TempSeas + PrepSeas + TH8 + BasinArea",
"PrepSeas ~ TH8",
"TempSeas ~ TH8")
...
spatialreg::errorsarlm(SR ~ BasinArea + TempSeas + PrepSeas + NPP + TempAnom + PrepAnom + EvolTime + SpecRate,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T),
# 2 # Equation 2: speciation rate as response
spatialreg::errorsarlm(SpecRate ~ BasinArea + TempSeas + PrepSeas + TempAnom + PrepAnom + TH8 + EvolTime,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T),
# 3 # Equation 3: evolutionary time as response  
spatialreg::errorsarlm(EvolTime ~ TempAnom + PrepAnom,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T),
# 4 # Equation 4: net primary productivity as response  
spatialreg::errorsarlm(NPP ~ TempSeas + PrepSeas + TH8 + BasinArea,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T),
# 5 # Equation 5: temperature seasonality as response  
spatialreg::errorsarlm(TempSeas ~ TH8,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T),
# 6 # Equation 6: precipitacion seasonality as response  
spatialreg::errorsarlm(PrepSeas ~ TH8,
                           data = vars,
                           listw = spatial_weights_d2,
                           tol=1e-12,zero.policy=T), 
data=vars)
...
### Running final pSEM 
# Delete non-significant paths and add missing correlate errors (check the summary model)

sem_sar_model_final <- piecewiseSEM::psem(
# 1 # Equation 1: species richness as response
spatialreg::errorsarlm(SR ~ BasinArea + TempSeas + NPP + TempAnom + EvolTime + SpecRate,
                         data = vars,
                         listw = spatial_weights_d2,
                         tol=1e-12,zero.policy=T),
# 2 # Equation 2: speciation rate as response
spatialreg::errorsarlm(SpecRate ~ EvolTime,
                         ...
                         tol=1e-12,zero.policy=T),
# 3 # Equation 3: evolutionary time as response  
spatialreg::errorsarlm(EvolTime ~ PrepAnom,
                         data = vars,
                         listw = spatial_weights_d2,
                         tol=1e-12,zero.policy=T),
# 4 # Equation 4: net primary productivity as response  
spatialreg::errorsarlm(NPP ~ TempSeas + PrepSeas + BasinArea,
                         data = vars,
                         listw = spatial_weights_d2,
                         tol=1e-12,zero.policy=T),
# 6 # Equation 6: precipitacion seasonality as response  
